	...
	namespace d3d11
	{
        ...
        void SwapChain::resize(int width, int height) {
            ...
            auto hr = swapchain_->ResizeBuffers(0, width, height, desc.BufferDesc.Format,
                desc.Flags);
            if (FAILED(hr)) {
                std::cout << "d3d11: Failed to resize swapchain (" << width << "x"
                    << height << ")" << std::endl;
                return;
            }
        ...
        std::shared_ptr<ID3D11Texture2D> Texture2D::get_shared_texture()
        {
            return texture_;
        }
        ...
        std::shared_ptr<Texture2D> Device::open_shared_texture(void* handle) {
            ...
            if (FAILED(hr)) {
                std::cout << "Failed to open D3D11_1 device. hr=" << std::hex << hr;
                return nullptr;
            }
            ...
            if (FAILED(hr)) {
                std::cout << "Failed to open shared texture. hr=" << std::hex << hr;
                return nullptr;
            }
        ...
        bool Device::create_shared_texture(DXGI_FORMAT format, uint32_t width, uint32_t height)
        {
            ...
            //////////////////////////////////////////////////////////////////////////////////////////
            // 设置D3D11的同享GPU模式  
            hr = shared_texture->QueryInterface(__uuidof(IDXGIResource), (void**)&dxgi_res);
        ...
        void Device::copy_shared_texture(ID3D11Texture2D*src)
        {
            ctx_->ctx_->CopyResource(shared_texture, src);
        }
...
float4 main(VS_OUTPUT input) : SV_Target
{
	return tex0.Sample(samp0, input.tex);
        ...
        void FrameBuffer::copy_shared_texture()
        {
            if (shared_buffer_ && device_)
            {
                device_->copy_shared_texture(shared_buffer_->get_shared_texture().get());
            }
        }
